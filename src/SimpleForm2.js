...
const validateWithMessage = {
  required: text =>
    (!validator.isValid.exists(text) &&
      validator.messageTemplate.error("Required")) ||
    validator.messageTemplate.NONE,
  email: email =>
    (!validator.isValid.email(email) &&
      validator.messageTemplate.error("Email is invalid")) ||
    validator.messageTemplate.NONE,
  emailConfirm: email => emailToCheck =>
    (email !== emailToCheck &&
      validator.messageTemplate.error("Email is not the same")) ||
    validator.messageTemplate.NONE,
  tweetLength: tweet =>
    (!validator.isValid.tweetLength(tweet) &&
      validator.messageTemplate.error(
        `${tweet.length - MAX_TWEET_LENGTH} character${tweet.length -
          MAX_TWEET_LENGTH ===
        1
          ? ""
          : "s"} over`
  ...
  render() {
    ...
    return (
      <div>
        ...
        />
        {message.body && (
          <InputMessage status={message.status}>{message.body}</InputMessage>
        )}
      </div>
    );
  }
...
export default class SimpleForm2 extends Component {
  ...
  setValidationMessage(stateKey, messageValidator) {
    ...
    this.setState(prevState => ({
      messages: {
        ...prevState.messages,
        [stateKey]: validator.validateAgainst(
          prevState[stateKey],
          messageValidator
        )
      }
  ...
  validationMessages(state) {
    const validationMessages = {
      ...
      emailConfirm: [
        validateWithMessage.required,
        validateWithMessage.emailConfirm(state.email)
      ]
    };
    ...
    return (
      <form
        ...
        onSubmit={e => {
          ...
          this.setState(
            prevState => ({
              messages: validator.validateAgainstMultiple(
                prevState,
                validationMessages
              )
            }),
        ...
        <VerticalSpacer space=".5em">
          <div>
            ...
            <Input
              name="name"
              className={
                this.state.messages.name.status === "error" && "invalid"
              }
              ...
              onChange={e => {
                ...
                this.setState(prevState => ({
                  ...
                  messages: {
                    ...prevState.messages,
                    name: validator.validateAgainst(
                      name,
          ...
          <LabeledInputWithValidation
            ...
            validate={() =>
              this.setValidationMessage("email", validationMessages.email)}
          />
          <LabeledInputWithValidation
            name="emailConfirm"
            label="Confirm Email"
            ...
            validate={() =>
              this.setValidationMessage(
                "emailConfirm",
                validationMessages.emailConfirm
              )}
          />
