...
export const ResponsiveOption = ({children, styles}) => (
	<div>{children}</div>
);
...
export default class ResponsiveSelect extends Component {
	...
	checkSize = () => {
		...
		const allFit = Array.of(...this.buttonGroup.children).every(child => child.scrollWidth <= child.clientWidth);
