...
describe("validateWithPromises()", () => {
    ...
    test("with multiple error messages", () => {
        ...
        return validateWithPromises(isValidPassword, "Password1").then(
            message =>
                expect(message).toEqual(
                    "You can't use the password 'Password1'"
                )
    ...
    test("with multiple error messages", () => {
        ...
        return validateWithPromises(isValidPassword, "Password1").then(
            message => expect(message).toEqual(NO_MESSAGE)
        );
    });
    test("with multiple error messages", () => {
        const isValidPassword = [
    ...
    test("non-promise with valid input", () => {
        return validateWithPromises(validations.required, "bla").then(
            message => {
                expect(message).toEqual(NO_MESSAGE);
            }
        );
    ...
    test("promise with valid input", () => {
        const requiredAsync = value =>
            Promise.resolve(validations.required(value));
...
describe("validateAllWithPromises()", () => {
  ...
  */
    ...
    test("propagate exceptions in promises", () => {
        ...
        return validateAllWithPromises(formValidations, formFields).catch(
            error => {
