...
export default class Validated extends Component {
		...
		this.setState((currState) => ({
			validationMessages: mapValues(this.getValidations(), (_, key) => {
				...
				if (!this.getValidations(currState)[key]) {
					throw new ReferenceError(`You're missing key: "${key}" in your validations prop in <Validated />`)
				}
	...
	allComplete = () => !includes(mapValues(this.getValidations(), (_, key) => this.getKeyValue(key)), '')
	...
	setValidationMessages = (validationMessages) => this.setState({ validationMessages })
	...
	render() {
		const inputProps = mapValues(this.getValidations(), (_, key) => ({
			...
			watch: (element) => (
				<element.type {...this.getProps(key)(element.props)} />
			),
		}));
...
export const ValidatedForm = ({ onSubmit, render, ...props }) => (
	<Validated {...props} render={({validateAll, ...args}) => (
		<form onSubmit={(e) => {
			e.preventDefault();
			validateAll(isValid => {
