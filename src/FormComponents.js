...
const statusColor = ((defaultColor, alpha = 1) => ({status}) => (
	status === 'error' && chroma('#f55280').alpha(alpha).css() ||
	status === 'warning' && chroma('#e8a538').alpha(alpha).css() ||
	status === 'success' && chroma('#5ad484').alpha(alpha).css() ||
	defaultColor
));
...
const FormControlBase = styled.input`
	...
	&:focus, &.focus {
		...
		box-shadow: inset 0px 2px 2px 0px ${statusColor('#2e7eff24', 0.14)}, 0px 0px 0px 2px ${statusColor('#2e5bff40', 0.25)};
	}
`;
...
export const Button = FormControlBase.withComponent('button').extend`
	font-weight: 500;
	// animation: ${({isSubmitting}) => isSubmitting && 'fade-out 1s ease-in-out infinite alternate'};
	...
	${({selected}) => selected && 'background-image: linear-gradient(#ececec, #bbbbbb0a);'}
`;
...
export class FadeOut extends Component {
	...
	render() {
		...
		const style = (this.props.isVisible
			? {opacity: spring(1, springConfig), width: spring(this.state.measuredWidth, springConfig)}
...
export const Loading = ({isVisible, style}) => (
	<span style={style}>
		<span style={{paddingLeft: '0', animation: 'fade-out2 1000ms 0ms ease-in-out infinite'}}>•</span>
		<span style={{paddingLeft: '0.15em', animation: 'fade-out2 1000ms 333ms ease-in-out infinite'}}>•</span>
		<span style={{paddingLeft: '0.15em', animation: 'fade-out2 1000ms 667ms ease-in-out infinite'}}>•</span>
	</span>
);
